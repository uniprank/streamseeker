...
class DownloaderFFmpeg:
    ...
    def _download_stream(self, ffmpeg_path, hls_url, file_name):  
        ...
        try:
            ...
            logger.info(f"<fg=green>Finished</> download of <fg=magenta>{file_name}</>")
            helper.download_success(file_name)
        except subprocess.CalledProcessError as e:
            logger.error(f"<fg=red>Server error</>. Could not download <fg=magenta>{file_name}</>. Please try to download it later.")
